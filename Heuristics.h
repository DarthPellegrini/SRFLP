...
float objectiveFunction(int size,int pos[],int flow[][size],float value[][2]){
    ...
    for(i=1; i<(size-1); i++){
        ...
        for(j=i+1; j<size; j++){
            ...
            result += (c+value[pos[i]][1]+value[pos[j]][1])*flow[pos[i]][pos[j]];
        }
    }
    return result;
} 
        ...
        for(i = 1; i < size-1; i+=(2*gsize)){
            ...
            while(i+c < size && c < 2*gsize){
                ...
                for(j = i; j < size && j < i+2*gsize; j++){
                    if(flow[pos[i]][pos[j]] > flow[pos[i]][pos[aux]] && wasUsed(used,pos[j]) == 0){
...
float heuristic2(int size, int pos[], int flow[][size],float value[][2],float last){
    /*
     *  VARIABLES
...
float hRecursion(int size, int pos[], int flow[][size],float value[][2],float last){
	//float aux = heuristic3(size,pos,flow,value,last);
